...
def grounding_dino_prompt(image, text):
    # Lazy import GroundingDINO and its transforms only when text prompting is used
    import sys
    import os
    
    # Add GroundingDINO path to sys.path if not already there
    groundingdino_path = os.path.join(os.path.dirname(__file__), 'dependencies', 'GroundingDINO')
    ...
    try:
        ...
        print(f"无法导入GroundingDINO: {e}")
        print("请确保GroundingDINO已正确安装")
        raise
    
    from torchvision.ops import box_convert

    def image_transform(image_pil: Image.Image) -> torch.Tensor:
        transform = T.Compose(
            [
    ...
    model_root = os.path.join(project_root, 'dependencies', 'GroundingDINO')

    model = load_model(
        os.path.join(model_root, "groundingdino/config/GroundingDINO_SwinT_OGC.py"),
        os.path.join(model_root, "weights/groundingdino_swint_ogc.pth")
    )

    # 提高阈值以减少误检，提高分割精度
    BOX_TRESHOLD = 0.40  # 从0.35提高到0.40
    TEXT_TRESHOLD = 0.30  # 从0.25提高到0.30
...
def compute_ratios(conv_2d, points_xy, indices_mask, sam_mask, h, w):
    # 确保所有张量在同一设备上
    target_device = points_xy.device
    sam_mask = sam_mask.to(target_device)
    conv_2d = conv_2d.to(target_device)
    ...
    vertex1 = torch.clip(vertex1, torch.tensor([0, 0]).to(target_device), torch.tensor([w-1, h-1]).to(target_device))
    vertex2 = torch.clip(vertex2, torch.tensor([0, 0]).to(target_device), torch.tensor([w-1, h-1]).to(target_device))
    ...
    ratios = torch.tensor(ratios, device=target_device)
...
def conv2d_matrix(gaussians, viewpoint_camera, indices_mask, device):
    ...
    w2c = viewpoint_camera.world_view_transform.to(device)
...
def update(gaussians, view, selected_index, ratios, dir_vector):
    # 确保所有张量在同一设备上
    target_device = gaussians.get_xyz.device
    ratios = ratios.unsqueeze(-1).to(target_device)
    ...
    conv3d_matrix = compute_conv3d(conv3d).to(target_device)
